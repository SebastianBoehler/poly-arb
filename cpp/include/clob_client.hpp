...
namespace polymarket
{
    ...
    public:
        ...
        // ============================================================
        // POSITION MANAGEMENT (Data API + CTF Contract)
        // ============================================================
        ...
        struct Position
        {
            ...
            double percent_pnl;
            double cur_price;
            bool redeemable; // Can redeem (market resolved, winning position)
            bool mergeable;  // Can merge Yes+No back to USDC
            std::string title;
            std::string slug;
        ...
        // Get only redeemable positions (resolved markets with winning shares)
        std::vector<Position> get_redeemable_positions(const std::string &user_address = "");

        // Get only mergeable positions (can merge Yes+No back to USDC)
        std::vector<Position> get_mergeable_positions(const std::string &user_address = "");

        // Redeem positions result
        struct RedeemResult
        {
            bool success;
            std::string tx_hash;
            std::string error_msg;
            std::string condition_id;
            double amount_redeemed; // USDC received
        };

        // Redeem winning positions for a resolved market
        // This calls the CTF contract's redeemPositions function
        RedeemResult redeem_positions(const std::string &condition_id);

        // Merge positions result
        struct MergeResult
        {
            bool success;
            std::string tx_hash;
            std::string error_msg;
            std::string condition_id;
            double amount_merged; // USDC received
        };

        // Merge Yes+No tokens back to USDC
        // Requires holding both Yes and No tokens for the same market
        MergeResult merge_positions(const std::string &condition_id, double amount);

        // Batch redeem all redeemable positions
        std::vector<RedeemResult> redeem_all_positions(const std::string &user_address = "");
