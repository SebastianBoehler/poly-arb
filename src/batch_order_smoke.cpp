...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i <= 3; i++)
    {
        ...
        if (!gamma_response.ok()) continue;
        
        auto gamma_json = json::parse(gamma_response.body);
        if (!gamma_json.is_array() || gamma_json.empty()) continue;
        
        auto &event = gamma_json[0];
        if (!event.contains("markets") || event["markets"].empty()) continue;
        ...
        if (!yes_book.ok() || !no_book.ok()) continue;
    ...
    // Calculate order amounts - use best ask prices exactly
    ...
    std::cout << "    Signing took: " << sign_duration.count() << " µs\n";
    
    // Build batch order payload
    json batch_payload = json::array();
    
    auto add_order = [](json &arr, const SignedOrder &order, const std::string &order_type)
    {
        json order_json;
        order_json["order"] = {
            {"salt", order.salt},
        ...
        order_json["orderType"] = order_type;
        arr.push_back(order_json);
    };
    
    // Use FOK (Fill-Or-Kill) to ensure immediate fill or cancel
    add_order(batch_payload, yes_signed, "FOK");
    add_order(batch_payload, no_signed, "FOK");
    
    std::cout << "\n[7] Posting batch order (FOK)...\n";
    
    std::string body_str = batch_payload.dump();
    auto l2_headers = signer.get_l2_headers(creds, "POST", "/orders", body_str);
