...
function parseSymbolFromSlug(slug: string): string {
  const first = slug.split("-")[0];
  if (!first) return "unknown";
  return first.toLowerCase();
}

export function get15mTimestamps(count: number): number[] {
...
export function formatHourET(hour: number): string {
...
export function generate1hSlugs(count: number): string[] {
  const slugs: string[] = [];
  const now = new Date();
  // Convert to ET (UTC-5)
  const etOffset = -5 * 60;
...
export function get4hTimestamps(count: number): number[] {
  const now = Math.floor(Date.now() / 1000);
    ...
    while (markets.length < maxMarkets) {
      ...
      for (const m of marketList) {
        ...
        markets.push({
          ...
          symbol: parseSymbolFromSlug(m.market_slug || m.question || "unknown"),
